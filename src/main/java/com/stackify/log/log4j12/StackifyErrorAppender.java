...
public class StackifyErrorAppender extends NonReentrantAppender {
	...
	public void activateOptions() {
		...
		try {
			this.logAppender = new LogAppender<LoggingEvent>("stackify-error-log4j12", new LoggingEventAdapter(apiConfig.getEnvDetail()));
	...
	protected void subAppend(final LoggingEvent event) {
		try {
			...
			errorHandler.error("Exception appending event to Stackify Error Appender", e, 0);
		}
	}
	...
	public void close() {
		try {
			...
			errorHandler.error("Exception closing Stackify Error Appender", e, 0);
		}
	}
